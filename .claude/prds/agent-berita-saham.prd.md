...
Contoh divergensi: filing `v2_filings.json` menyatakan `effect_date` 2026-09-01, tapi `v2_company_corporate-actions_*` menunjukkan `recorded_at` 2026-09-03 untuk aksi yang sama. Tanpa sitasi wajib (endpoint + field + `as_of` + rumus), analyst tidak bisa membedakan mana yang benar — dan risiko halusinasi meningkat saat LLM menyusun klaim dari dua sumber yang berbeda. Ini bukan masalah abstrak; ini divergensi yang bisa dibuktikan dari `recorded/`.
...
**Dalam:**
1. Poll `/v2/filings/` (`?since=` kontrol) — 1 kredit/hari × 19 hari = 19 kredit
2. Peta filing → simbol → konsentrasi broker (`/v2/broker-summary/{sym}/`, `?sections=overview`) — 1 kredit/hari × 19 = 19 kredit (hanya simbol terverifikasi dari tier sebelumnya)
...
4. **Gate anti-halusinasi dengan comparator**: `gate.py` (seperti `gate.comparator_mislabelled`) — draft klaim dibandingkan angka endpoint (`pnl_pct`, `mcap`, `tvl`, `volume`). Flag mislabel: angka tidak cocok, field salah, `as_of` berbeda. Draft ditolak jika `mismatch_score > 0`.
...
- [x] Data 30 event filing: `recorded/v2_filings.json` = 23472 entry (>30 cukup); `v2_company_corporate-actions_*` = 5 file (verifikasi isi diperlukan tapi tidak blokir milestone 3).
