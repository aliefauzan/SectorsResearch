...
- `mersamur/riset/temuan-kelayakan.md` §Q1 dan §"Uji berikutnya"
- `mersamur/plans/plan-feasibility.json` — tiru pola plannya
...
3. **Rehearse ke mock lebih dulu**, lalu hapus apa pun yang mock tulis ke
   ...
   cd ../research/harness && SECTORS_BASE_URL=http://127.0.0.1:8787 python3 src/capture.py --plan ../mersamur/plans/plan-control.json --budget 400
   ```

4. Baru live, dengan cap:

   ```bash
   cd ../research/harness && python3 src/capture.py --plan ../mersamur/plans/plan-control.json --budget 330
   ```
