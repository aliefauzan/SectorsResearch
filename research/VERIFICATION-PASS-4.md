...
| **Correct** | **That sentence appears nowhere under `99-raw/`.** |
| **Source** | `grep -r "replace human\|efficiency and accuracy" 99-raw/` → zero hits. The capture's actual annotation is `**Reviewed by humans** — human-in-the-loop on every AI step; "led by banking veterans and former data journalists"` (`sectors-api-for-idx-and-dataops.md:70`) |
...
`03-mock-data/README.md` claims the meter charges "the documented per-endpoint cost". Three
...
agent-skills reference page (`llms-full.txt:10629`, `99-raw/agent-skills/SKILL.md:282`) gives
...
| 9 | Declared costs | Cost sentence extracted from all 70; machine-diffed against every path+number line in prose, `plan.json` and `04-build-plan/` | **49 flat-1, 4 flat-2 (`most-traded`, `brokers/top`, `broker-summary/{}/top`, `broker-activity/{}/top`), 17 non-flat.** 70 flat-cost line checks, 1 flagged, that one a false positive (three paths on one line) |
...
| 13b | `is null` | Grepped the whole raw corpus | **One hit in all of `99-raw/`** — `llms-full.txt:7495`, a Google Sheets recipe ("exclude any rows where the `Symbol` field is null"). Not the query language. Pass 3's removal was **correct** |
...
| 19 | Internal links | 181 relative links across 45 markdown files | **Zero broken** outside fenced code blocks. The 3 dangling links in `99-raw/agent-skills/SKILL.md` are inside a verbatim third-party capture |
...
[`99-raw/pass4-live-recheck-2026-09-05.md`](99-raw/pass4-live-recheck-2026-09-05.md) §9;
...
All three are recorded in `03-mock-data/README.md` under "What it deliberately does not
