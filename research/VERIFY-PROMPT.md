...
- `research/01-hackathon/` — overview, rules, tracks, submission checklist.
- `research/02-sectors-platform/` — 16 docs. `02-endpoint-reference.md` and `07-response-shapes.md` are **generated** from `99-raw/schema.json`; the generators live in `99-raw/scripts/`.
- `research/03-mock-data/` — `extract_fixtures.py`, `mock_server.py` (credit meter + recording replay), `synth_universe.py`, `synth_extended.py`, `plan.json` (87 calls / 176 credits / 5 tiers), `capture.py` (idempotent budget-capped fetch harness).
- `research/04-build-plan/` — build ideas, competitive landscape, already-published work.
- `research/99-raw/` — 22 evidence files, including the full OpenAPI spec and `llms-full.txt`.
...
1. **Regenerate both generated docs** from `99-raw/schema.json` using the scripts in `99-raw/scripts/` and diff against the committed versions. Any drift is a finding.
...
8. **Quotes.** Every quoted string in the corpus must appear verbatim in a file under `99-raw/`. Grep each one. Report any quote with no saved provenance, and any that has drifted from its source.
...
- Re-sweep the hackathon site and `sectors.app` for anything added or changed since the corpus was written; diff against `99-raw/`.
