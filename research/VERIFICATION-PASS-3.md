...
| **Source** | Spec's *Syntax and Operators* accordion lists only `=`, `!=`, `>`, `>=`, `<`, `<=`, `like`, `in`. The Deterministic Query Builder capture attests `is not null` three times. Grepping the entire `99-raw/` corpus for `is null` returns **one** hit — `llms-full.txt:7495`, in a Google Sheets recipe ("exclude any rows where the `Symbol` field is null"), not the query language. |
...
the concatenated `99-raw/` corpus with punctuation normalised, then hand-classified the misses
...
1. **`BASE` was hardcoded to `https://api.sectors.app`** with no flag and no env var — despite
   `03-mock-data/README.md` recommending exactly that switch (`SECTORS_BASE_URL`) for user
...
- `03-mock-data/README.md` showed a worked `__usage` example under `--credits 1000` reading
...
I also re-derived the 4 Sep figures from `99-raw/hack-matching.md` and they match the dossier's
...
| # | Check | Method | Result |
| --- | --- | --- | --- |
| 1 | Generated docs reproducible | Ran both scripts in `99-raw/scripts/`, diffed against committed | **byte-identical**, both |
...
| 3 | Credit costs | Extracted the credit sentence from all 70 endpoint descriptions; diffed against every cost stated in prose, `plan.json` and `04-build-plan/` | 49 flat-1, exactly 4 two-credit (`most-traded`, `brokers/top`, `broker-summary/{}/top`, `broker-activity/{}/top`), 17 non-flat. One prose mismatch (§1.2) |
...
| 8 | Quotes | 165 quoted strings ≥15 chars grepped against `99-raw/` | 3 drifted (§1.7), 4 unsourced (§1.8), remainder trace |
...
| 13 | Internal links | All 43 markdown files | **Zero broken relative links** in the dossier. 3 dangling links exist inside `99-raw/agent-skills/SKILL.md`, which is a verbatim third-party capture |
...
451 KB. Saved to [`99-raw/postman/`](99-raw/postman/Sectors_API.postman_collection.json).
...
File tree of `sectors-mcp` saved to [`99-raw/sectors-mcp-repo-tree.txt`](99-raw/sectors-mcp-repo-tree.txt).
