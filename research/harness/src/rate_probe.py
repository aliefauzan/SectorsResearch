...
assumption before anything else runs, and the whole probe aborts if it turns out false.
    ...
    python3 src/rate_probe.py --phase0            # is the free path rate limited at all?
    python3 src/rate_probe.py                     # full sweep across spacings
    python3 src/rate_probe.py --confirm 0.5       # spend a few credits confirming one spacing
...
LOG = os.path.join(ROOT, "recorded", "_rate_probe.jsonl")
