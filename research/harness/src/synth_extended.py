...
`plan/` actually need, in the same field naming the live API uses:
...
Run `synth_universe.py` first — this reads its `companies.json` and `daily/`.
    ...
    python3 src/synth_universe.py --companies 200 --days 180
    python3 src/synth_extended.py
...
# docs/api/06-parameter-cheatsheet.md.
...
def make_broker_top(rng, companies, days, n_symbols):
    ...
    docs/api/10-domain-pitfalls.md. Keeping the property here means a
...
ROUTE = {'banking.json': 'company',
         'suspensions.json': 'company',
         'corporate_actions.json': 'company',
         'segments.json': 'company',
         'listing_performance.json': 'company',
         'quarterly_financial_dates.json': 'company',
         'shareholders_composition.json': 'flow',
         'filings.json': 'flow',
         'brokers.json': 'flow',
         'free_float.json': 'market',
         'idx_total.json': 'market',
         'trading_days.json': 'market'}


def main():
    here = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))   # harness/
    ...
    companies_path = os.path.join(args.indir, "market", "companies.json")
    ...
    for sub in ("mining", "market", "flow", "company",
                os.path.join("company", "quarterly"),
                os.path.join("market", "index_daily"),
                os.path.join("flow", "broker_top")):
    ...
    # Each dataset lands in the subdirectory its endpoint family belongs to; see the
    # layout table in README.md. Keys stay bare filenames so the summary below reads.
    for name, payload in simple.items():
        json.dump(payload, open(os.path.join(args.out, ROUTE[name], name), "w"))

    for symbol, rows in quarterly.items():
        json.dump(rows, open(os.path.join(args.out, "company", "quarterly", symbol.replace(".JK", "") + ".json"), "w"))
    for code, series in indices.items():
        json.dump(series, open(os.path.join(args.out, "market", "index_daily", code + ".json"), "w"))
    for symbol, payload in broker_top.items():
        json.dump(payload, open(os.path.join(args.out, "flow", "broker_top", symbol.replace(".JK", "") + ".json"), "w"))
