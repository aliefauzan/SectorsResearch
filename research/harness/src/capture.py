...
response to disk, and develop against the recording from then on.
    ...
    python3 src/capture.py --plan plans/plan.json --dry-run          # cost it, call nothing
    python3 src/capture.py --plan plans/plan.json --tier 0           # helper lists, 5 credits
    python3 src/capture.py --plan plans/plan.json --tier 0 --tier 1  # + core, ~45 credits
    python3 src/capture.py --plan plans/plan.json --budget 200       # hard cap
    python3 src/capture.py --plan plans/plan.json --report           # what has been spent
...
ROOT = os.path.dirname(HERE)          # harness/ — data and plans live beside src/
...
def main():
    ...
    ap.add_argument("--plan", default=os.path.join(ROOT, "plans", "plan.json"))
