...
Three checks, in order of how much they would cost to get wrong:
  ...
  3. **Header parity.** The live API returns no spend headers. Neither may the mock, unless
    ...
    python3 src/verify_mock.py                  # starts its own server on a free port
    python3 src/verify_mock.py --port 8787      # or check one already running
...
# string where one was captured. See ../evidence/live-capture-2026-09-06.md.
LIVE_BEHAVIOUR = [
