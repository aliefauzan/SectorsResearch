...
only independent record — one row per request, with the credits actually charged.
    ...
    python3 src/reconcile_usage.py                       # uses ../../evidence/usage-log/*.csv
    python3 src/reconcile_usage.py ~/Downloads/*.csv     # or point it at a fresh export
...
LEDGER = os.path.join(ROOT, "recorded", "_ledger.jsonl")
DEFAULT_LOGS = os.path.join(ROOT, "..", "evidence", "usage-log", "*.csv")
