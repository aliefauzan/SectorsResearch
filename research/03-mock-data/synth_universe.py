...
Deterministic: the same --seed always produces the same universe.
    ...
    python3 synth_universe.py --companies 300 --days 180 --out synth/
...
def main():
    ...
    for company in companies:
        ...
        with open(os.path.join(args.out, "daily", ticker + ".json"), "w") as fh:
            json.dump(series, fh)
    ...
    outputs = {
        ...
        "foreign_flow.json": foreign_flow,
        "broker_summary.json": broker_rows,
        "news.json": make_news(rng, companies, args.news, [str(d) for d in days]),
        "subsectors.json": [{"sector": s, "sub_sector": ss} for s, ss, _ in TAXONOMY],
    }
    ...
    print("  %d price points · %d broker rows · %d news items"
          % (len(companies) * len(days), len(broker_rows), len(outputs["news.json"])))
