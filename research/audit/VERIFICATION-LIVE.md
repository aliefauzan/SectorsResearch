...
Evidence: `harness/recorded/` (payloads, `_manifest.json`, `_ledger.jsonl`) and
...
[`harness/src/plan_live.py`](../harness/src/plan_live.py) reads them out of `recorded/` and
...
it. `harness/src/verify_mock.py` reruns the whole audit for zero credits and exits non-zero
on any divergence.
...
Every `/v2/…` string in the whole `evidence/` capture set was also resolved: the
...
[`evidence/usage-log/`](../evidence/usage-log/). `harness/src/reconcile_usage.py` diffs them against
