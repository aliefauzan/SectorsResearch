...
`evidence/spec/schema.json`, the public Postman collection, `llms-full.txt`, and the `sectors-mcp`
...
- **Every live call goes through `research/harness/src/capture.py`.** No ad-hoc `curl`, no
...
`docs/api/05-credit-budget.md` and to the mock's cost model.
...
synthetic generators in `harness/` can stand in for a full universe sweep, and no other
tier can.
...
`research/evidence/live-recheck-<date>.md`, keep the payloads under
`research/harness/recorded/`, and add a row to the audit section of `research/README.md`.
