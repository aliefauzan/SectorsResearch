...
Three files implement it, all in [`harness/`](../../harness/):
...
```bash
python3 src/capture.py --base-url http://localhost:8787 --tier 0   # full rehearsal, 0 credits
python3 src/capture.py --dry-run                  # cost it, call nothing
python3 src/capture.py --tier 0                   # 5 credits, reconcile the model
python3 src/capture.py --tier 0 --tier 1          # 33 credits, enough to build
python3 src/capture.py --budget 200               # everything, hard-capped
python3 src/capture.py --report                   # what has actually been spent
```
...
| **Configuration from `.env`** | `SECTORS_API_KEY`, `SECTORS_BASE_URL` and `SECTORS_BUDGET` are read by [`sectors_env.py`](../../harness/src/sectors_env.py) from the git-ignored `.env` at the repository root, so every teammate runs the same configuration without exporting anything. A real environment variable still wins over the file. The key is never logged and never written to disk — the run header prints only `key: set` or `key: MISSING`. |
...
```bash
python3 src/capture.py --report      # compare est_cost against observed cost headers
```
